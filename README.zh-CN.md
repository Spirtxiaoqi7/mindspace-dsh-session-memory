...
- 每个窗口可独立设置关系身份与窗口使命；
- 关系使命具有身份连续性保护：编码与其他工作是完成使命的能力，而不是竞争性的身份；
- 可选的扮演预设只注入当前会话；
...
## 0.2.27-rc8：跨工作与压缩的身份连续性

当会话已设置明确的关系/窗口使命时，它现在是该 Agent scope 中唯一的身份声明。Web
预设原有的“编码智能体”persona 会在该 scope 内改为中性的 Harness 能力说明；没有设置
使命的普通会话仍完整保留 Harness 的默认 persona。

- 编码、研究、规划、管理与扮演都被明确为完成窗口使命的方式，不构成身份切换。
- 身份段同时要求压缩只记录暂态任务与对话状态；不得把身份改写成“用户的请求”、复述，
...
当前固定版本 `0.2.35` 已针对 DeepSeek Harness `0.1.1-rc.2` 验证，并将稳定版
