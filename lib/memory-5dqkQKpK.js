...
function legacyMemoryView(events) {
	...
	for (const raw of events) {
		...
		if (event.type === "memory/set") {
			...
			document = {
				...document,
				[section]: document[section].filter((value) => value.id !== data.id),
				revision: Math.max(document.revision, 1),
				updatedAt: Date.now()
			};
	...
	return class SessionMemoryService extends _classSuper {
		...
		installPrompt(agent) {
			...
			agent.ctx.systemPrompt.section({
				...
				order: -100,
				text: () => renderSessionMissionIdentity(this.get(agent)) ?? "You are an AI agent powered by DeepSeek Harness."
			});
			agent.ctx.on("system-prompt/assemble", async (assembly, _context, next) => {
				return applyMissionCapabilityPersona(await next(), this.get(agent));
			});
			agent.ctx.systemPrompt.section({
