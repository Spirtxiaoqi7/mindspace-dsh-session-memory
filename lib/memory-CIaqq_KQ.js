...
function applyMutation(mode, args, sourceSeqs) {
	if (args.assistant_setting !== void 0 || args.assistant_state !== void 0) return {
		...mode,
		assistantSetting: args.assistant_setting ?? mode.assistantSetting,
		assistantState: args.assistant_state ?? mode.assistantState
	};
...
function instruction(args) {
	if (args.assistant_setting !== void 0) return `更新 AI 设定：${args.assistant_setting}`;
	if (args.assistant_state !== void 0) return `更新 AI 当前状态：${args.assistant_state}`;
...
let SessionMemoryService = (() => {
	...
	return class SessionMemoryService extends _classSuper {
		...
		currentTurn(agent) {
			return agent.session.events.findLast((event) => event.type === "turn/start")?.data.turn ?? 0;
		}
		registerTools() {
			...
			this.ctx.tools.register(defineTool({
				...
				description: "Read both memory faces, active mode, neutral bridge, and activity before a memory write.",
				parameters: {},
				output: {
					schema: { type: "json" },
					render: (_args, value) => [{
				...
				execute: (_args, exec) => {
					...
					this.modelReadState.set(String(exec.agent.id), {
						...
						turn: this.currentTurn(exec.agent)
					});
			...
			this.ctx.tools.register(defineTool({
				...
				description: "Write the active mode, or stage a cross-mode write in the neutral bridge. Call get_session_memory first.",
				...
				execute: async (raw, exec) => {
					...
					const read = this.modelReadState.get(String(exec.agent.id));
					...
					if (args.target_mode === current.activeMode) next = {
						...request,
						[args.target_mode]: applyMutation(current[args.target_mode], args, sourceSeqs)
					};
					else {
						const pending = {
...
//#endregion
export { renderSessionMemoryContext as a, emptySessionMemory as c, migrateLegacyDocument as d, migrateV4Document as f, renderSessionMemory as i, emptySessionMemoryFoldState as l, renderAssistantRequirements as n, applySessionMemoryEvent as o, sessionMemoryView as p, renderBridge as r, emptyModeMemory as s, SessionMemoryService as t, foldSessionMemory as u };
