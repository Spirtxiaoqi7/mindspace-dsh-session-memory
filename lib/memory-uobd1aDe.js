	...
	return class SessionMemoryService extends _classSuper {
		...
		registerTools() {
			...
			this.ctx.tools.register(defineTool({
				...
				description: "Persist explicit personalization now. Match item cards by category; exact item ids are optional. Merge related details and replace conflicts instead of creating sentence-shaped duplicate cards. Use userProfile only for identity/location/work/skills/life state; preferences for likes/dislikes/choices; assistantInstructions for rules governing assistant answers and behavior.",
				parameters: {
					action: {
						type: "string",
						required: true,
					...
					text: {
						type: "string",
						description: "Complete consolidated card or roleplay preset text."
					},
					item_id: {
						type: "string",
