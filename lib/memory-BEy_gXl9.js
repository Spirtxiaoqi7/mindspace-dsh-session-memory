...
function applySessionMemoryEvent(state, event) {
	if (event.type === "mindspace-compaction/policy") {
		...
		if (value.enabled !== void 0 && Number.isFinite(value.thresholdRatio) && Number.isInteger(value.retainTokens) && Number.isInteger(value.maxTokens)) return {
			...state,
...
let SessionMemoryService = (() => {
	...
	return class SessionMemoryService extends _classSuper {
		...
		async commit(agent, request, sourceSeqs) {
			...
			if (changes.length === 0 && !policyChanged) return {
				ok: true,
			...
			if (policyChanged) agent.session.append("mindspace-compaction/policy", {
				version: 1,
...
//#endregion
export { DEFAULT_RELATIONSHIP_MISSION as a, mergeExtraction as c, applySessionMemoryEvent as d, emptySessionMemory as f, sessionMemoryView as g, migrateLegacyDocument as h, DEFAULT_PROFILE_CHARACTERS as i, parseExtraction as l, foldSessionMemory as m, renderSessionMemory as n, EXTRACTION_SYSTEM as o, emptySessionMemoryFoldState as p, renderSessionMissionIdentity as r, MAX_MEMORY_CARDS as s, SessionMemoryService as t, turnExtractionInput as u };
