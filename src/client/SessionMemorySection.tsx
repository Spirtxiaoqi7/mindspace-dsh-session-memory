...
import type { ContextCompactionPolicy, ReplaceSessionMemoryRequest, SessionMemoryItem, SessionMemoryMode, SessionMemoryMutationResult, SessionMemoryView, SessionModeMemory, SessionPerson } from '../memory/types.ts'
...
export interface SessionMemorySectionInjected { remote: SessionMemoryRemote; t: (key: SessionMemoryKey) => string }
...
export function SessionMemorySection({ useSessions, useWorkspaces, remote, t }: SessionMemorySectionProps) {
  if (!remote || !t) return null
  const sessions = useSessions(state => state); const workspaces = useWorkspaces(state => state)
  const [selectedId, setSelectedId] = useState<string | undefined>(sessions.current ?? sessions.ids[0]); const [view, setView] = useState<SessionMemoryView>(); const [draft, setDraft] = useState<Draft>(); const [tab, setTab] = useState<SessionMemoryMode>('chat'); const [status, setStatus] = useState('')
  ...
  return <div className={css.section}><header><h2>记忆中心</h2><p>同一个人、同一个 AI，按当前任务载入 Chat 或 Work。模式不限制工具，只隔离记忆与写入目标。</p></header><label className={css.sessionSelect}><span>会话</span><select value={selected} onChange={event => { setSelectedId(event.target.value); setView(undefined); setDraft(undefined) }}>{options.map(row => <option key={row!.id} value={row!.id}>{row!.displayTitle}</option>)}</select></label>
    {draft && <><div className={css.modeTabs}><button className={tab === 'chat' ? css.activeTab : ''} onClick={() => setTab('chat')}>Chat</button><button className={tab === 'work' ? css.activeTab : ''} onClick={() => setTab('work')}>Work</button><span>当前注入：{draft.activeMode === 'chat' ? 'Chat' : 'Work'} · {draft.modeSource === 'model' ? '模型判断' : draft.modeSource === 'user' ? '用户选择' : '迁移默认'}</span></div><ModeEditor mode={tab} value={draft[tab]} onChange={value => setDraft({ ...draft, [tab]: value })}/>
      ...
      <section className={css.card}><h3>上下文压缩</h3><label className={css.switch}><input type="checkbox" checked={draft.policy.enabled} onChange={event => setDraft({ ...draft, policy: { ...draft.policy, enabled: event.target.checked } })}/><span>{draft.policy.enabled ? '已启用' : '已关闭'}</span></label><div className={css.twoColumn}><label><span>触发比例 %</span><input type="number" min="5" max="80" step="0.1" value={Number((draft.policy.thresholdRatio * 100).toFixed(1))} onChange={event => setDraft({ ...draft, policy: { ...draft.policy, thresholdRatio: Number(event.target.value) / 100 } })}/></label><label><span>保留 tokens</span><input type="number" min="4096" value={draft.policy.retainTokens} onChange={event => setDraft({ ...draft, policy: { ...draft.policy, retainTokens: Number(event.target.value) } })}/></label></div></section>
