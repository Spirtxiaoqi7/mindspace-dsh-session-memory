...
function Cards({ title, items, onChange }: { title: string; items: readonly SessionMemoryItem[]; onChange: (items: SessionMemoryItem[]) => void }) {
  return <section className={css.subcard}><div className={css.cardTitle}><h4>{title}</h4><span className={css.limitBadge}>{items.length}/100</span></div>
    ...
    <button className={css.addGroupButton} type="button" disabled={items.length >= 100} onClick={() => onChange([...items, card()])}>新增归纳组</button></section>
}
...
function ModeEditor({ mode, value, onChange }: { mode: SessionMemoryMode; value: SessionModeMemory; onChange: (value: SessionModeMemory) => void }) {
  ...
  return <div className={css.modePanel}><div className={css.modeHeading}><div><h3>{label}</h3><p>{mode === 'chat' ? '日常关系、偏好、长期经历与当前状态。' : '项目、工程状态、协作关系与工作要求。'}</p></div><span>{value.people.length}/5 人物</span></div>
    <section className={css.subcard}><label><span>AI 设定</span><textarea rows={3} value={value.assistantSetting} onChange={event => onChange({ ...value, assistantSetting: event.target.value })}/></label><label><span>{mode === 'chat' ? 'AI 当前状态' : 'AI 当前工作角色与状态'}</span><textarea rows={2} value={value.assistantState} onChange={event => onChange({ ...value, assistantState: event.target.value })}/></label></section>
...
export function SessionMemorySection({ useSessions, useWorkspaces, remote, commands, inheritance, t }: SessionMemorySectionProps) {
  ...
  const inheritSettings = async () => {
    ...
    try {
      ...
      if (!saved.ok) { setStatus(saved.error.message); return }
      if (!saved.value.ok) { setStatus(saved.value.error.message); return }
      ...
      if (!copied.ok) { setStatus(`新会话已创建，但设定继承失败：${copied.error.message}`); return }
      if (!copied.value.ok) { setStatus(`新会话已创建，但设定继承失败：${copied.value.error.message}`); return }
